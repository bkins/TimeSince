...
{
    private Frame          PrivacyPolicyFrame       { get; set; } = null!;
    private Frame          AwesomePersonFrame       { get; set; } = null!;
    private Frame          SupportMeFrame           { get; set; } = null!;
    private Frame          VersionFrame             { get; set; } = null!;
    private Frame          DescriptionFrame         { get; set; } = null!;
    private Frame          DevBioFrame              { get; set; } = null!;
    private Label          EmailLabel               { get; set; } = null!;
    private AboutViewModel AboutViewModel           { get; set; }
    private View?          AdView                   { get; set; }
    private Button         ShowRewardAdButton       { get; set; } = null!;
    private Button         ShowInterstitialAdButton { get; set; } = null!;
    private Button         ResetAwesomeScoreButton  { get; set; } = null!;
    private Button         PrivacyPolicyButton      { get; set; } = null!;
    private Frame          HeaderFrame              { get; set; } = null!;
    ...
    private void SetButtonsTextColor()
    {
        ...
        PrivacyPolicyButton.TextColor      = buttonTextColor ?? Colors.Black;
        ResetAwesomeScoreButton.TextColor  = buttonTextColor ?? Colors.Black;
        ShowInterstitialAdButton.TextColor = buttonTextColor ?? Colors.Black;
        ShowRewardAdButton.TextColor       = buttonTextColor ?? Colors.Black;
    }

    private void RefreshAdViewOnPage()
    {
        if (AppIntegrationService.IsPhysicalDevice()) return;

        ShowInterstitialAdButton.IsEnabled = false;
        ShowRewardAdButton.IsEnabled       = false;

        Logger.ToastMessage("Ads cannot be shown on emulated devices.");
    }
    ...
    private void DefinePrivacyPolicyFrame(Style frameStyle)
    {
        ...
        if (PrivacyPolicyFrame.Content is not Grid) return;
                                 ...
                                 Content = new StackLayout
                                           {
                                               Children =
                                               {
                                                   ...
                                                   {
                                                       Text =
                                                           "You are already an awesome person! This number just shows how awesomer you are."
                                                       ...
                                                       Content = new StackLayout
                                                                 {
                                                                     Children =
                                                                     {
                                                                         new Label
                                                                         {
                                                                           ...
                                                                           , TextColor =
                                                                                 ColorUtility.GetColorFromResources(
                                                                                     ResourceColors.Tertiary)
                                                                              ?? Colors.Black
    ...
    private void DefineVersionFrame(Style frameStyle)
    {
        ...
        ((Grid)VersionFrame.Content).Add(new Label
                                         {
                                             Text      = AboutViewModel.CurrentVersion ?? "0"
                                           , FontSize  = 16
                                           , TextColor = ColorUtility.GetColorFromResources(ResourceColors.Tertiary) ?? Colors.Black
                                           , Margin = new Thickness(5
                                                                  , 0
        ...
        ((Grid)VersionFrame.Content).Add(new Label
                                         {
                                             Text      = AboutViewModel.CurrentBuild ?? "0"
                                           , FontSize  = 16
                                           , TextColor = ColorUtility.GetColorFromResources(ResourceColors.Tertiary) ?? Colors.Black
                          ...
                          Content = new StackLayout
                                    {
                                        Children =
                                        {
                                            ...
                                            {
                                                Text =
                                                    "Hi! My name is Ben. I create apps like this in my spare time. I do this mostly for fun and to keep my skills up. I also do it to bring in a bit of supplemental income."
    ...
    {
        if (Application.Current?.Resources.TryGetValue("CommonFrameStyle"
                                                     , out var style)
         ?? false)
        {
            return (Style)style;
